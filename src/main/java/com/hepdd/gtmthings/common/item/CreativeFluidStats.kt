...
class CreativeFluidStats :
    ...
    override fun appendHoverText(stack: ItemStack, level: Level?, tooltipComponents: MutableList<Component?>, isAdvanced: TooltipFlag?) {
        ...
        if (stack.hasTag() && stack.tag!!.contains("Fluid")) {
            FluidUtil.getFluidContained(stack).ifPresent(
                Consumer { tank: FluidStack? ->
                    tooltipComponents
    ...
    override fun createUI(holder: HeldItemHolder?, entityPlayer: Player): ModularUI = ModularUI(176, 166, holder, entityPlayer)
        .widget(createWidget())
        .widget(
    ...
    private fun getStored(fluidCell: ItemStack): FluidStack {
        val tagCompound = fluidCell.tag
        return if (tagCompound != null && tagCompound.contains("Fluid")) {
