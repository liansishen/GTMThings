    ...
    public InteractionResult onItemUseFirst(ItemStack itemStack, UseOnContext context) {
        if (context.getPlayer() != null && context.getPlayer().isShiftKeyDown()) {
            ...
            if ((itemStack.is(CustomItems.WIRELESS_ITEM_TRANSFER_COVER.asItem())
                ...
                if (level.isClientSide()) player.sendSystemMessage(Component.translatable("item.gtmthings.wireless_transfer.tooltip.bind.1"
                        ,Component.translatable(level.getBlockState(blockPos).getBlock().getDescriptionId())
