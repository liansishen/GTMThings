...
open class HugeDualHatchPartMachine(holder: IMachineBlockEntity, tier: Int, io: IO):HugeBusPartMachine(holder, tier, io, 9) {
    ...
    protected fun createTank(): NotifiableFluidTank {
        return object : NotifiableFluidTank(this, this.getTankInventorySize(), Int.Companion.MAX_VALUE, io) {
    ...
    protected fun getTankInventorySize(): Int {
    ...
    private fun addDisplayText(textList: MutableList<Component?>) {
            ...
            if (!fs.isEmpty) {
                textList.add(
                    fs.displayName.copy().setStyle(Style.EMPTY.withColor(ChatFormatting.GOLD))
                        .append(
                            Component.literal(
                                if (fs.amount < 1000) fs.amount
                                    .toString() + "mB" else FormatUtil.formatNumber(fs.amount / 1000L) + "B"
                            )
                                .setStyle(Style.EMPTY.withColor(ChatFormatting.AQUA))
                        )
                )
        ...
        textList.add(
            0, Component.translatable("gtmthings.machine.huge_item_bus.tooltip.2", itemCount, super.getInventorySize())
