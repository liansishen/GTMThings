...
open class WirelessEnergyMonitor(holder: IMachineBlockEntity) :
    MetaMachine(holder),
    IFancyUIMachine,
    IWirelessMonitor {
    ...
    override fun isRemote(): Boolean = if (holder.level() == null) GTCEu.isClientThread() else holder.level().isClientSide
    ...
    override fun getUUID(): UUID? = this.ownerUUID

    override fun display(): Boolean = false

    override fun getMonitorLevel(): Level? = holder.level()

    override fun getWirelessEnergyContainerCache(): WirelessEnergyContainer? = this.cache
